...
export const useCourses = () => {
  ...
  const [course, setCourse] = useState(null); // Add single course state
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err.message || "Une erreur est survenue lors du chargement des cours"
      );
      return null;
    } finally {
      setLoading(false);
  ...
  const fetchCoursesByStudyModule = useCallback(async (studyModuleId) => {
    ...
    try {
      ...
      setError(
        err.message ||
          `Une erreur est survenue lors du chargement des cours pour le module ${studyModuleId}`
      );
      return { err };
    } finally {
      setLoading(false);
  ...
  const createCourse = useCallback(
    async (courseData) => {
      ...
      try {
        ...
        setError(
          err.message || "Une erreur est survenue lors de la création du cours"
        );
        return null;
      } finally {
        setLoading(false);
  ...
  const getCourseById = useCallback(async (id) => {
    ...
    try {
      ...
      setCourse(courseData); // Set the single course state
      return courseData;
    } catch (err) {
      console.error("Error fetching course:", err);
      setError(
        err.message ||
          `Une erreur est survenue lors de la récupération du cours ${id}`
      );
      setCourse(null);
      return null;
    } finally {
  ...
  const updateCourse = useCallback(
    async (id, data) => {
      ...
      try {
        ...
        // Update the single course state if it's the same course
        ...
        // Optionally refresh all courses if needed
        // fetchCourses();
        ...
        setError(
          err.message ||
            `Une erreur est survenue lors de la mise à jour du cours ${id}`
        );
        return null;
      } finally {
        setLoading(false);
  ...
  const deleteCourse = useCallback(
    async (id) => {
      ...
      try {
        ...
        // Clear single course if it's the deleted one
        if (course && course.id === id) {
          setCourse(null);
        }
        ...
        setError(
          err.message ||
            `Une erreur est survenue lors de la suppression du cours ${id}`
        );
        return null;
      } finally {
        setLoading(false);
  ...
  const getCoursesByStudyModule = useCallback(
    async (studyModuleId) => {
      try {
  ...
  const enrollInCourse = useCallback(
    async (courseId) => {
      ...
      try {
        ...
        setError(
          err.message ||
            `Une erreur est survenue lors de l'inscription au cours ${courseId}`
        );
        return null;
      } finally {
  ...
  return {
    courses,
    course, // Add single course to return
    loading,
    error,
    enrollmentLoading,
    fetchCourses,
